...
int8_t a_data[M * K];
MemRefDescriptor<int8_t, 2> a = {a_data, a_data, 0, {M, K}, {K, 1}};

int8_t b_data[K * N];
MemRefDescriptor<int8_t, 2> b = {b_data, b_data, 0, {K, N}, {N, 1}};

int32_t result_data[M * N];
MemRefDescriptor<int32_t, 2> result = {
    result_data, result_data, 0, {M, N}, {N, 1}};
...
int main() { _mlir_ciface_gemm(result, a, b); }
