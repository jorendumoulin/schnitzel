...
class Cluster extends Module {
  ...
  val interconnect = Module(new Interconnect(2, numBanks, CoreConfig.addrWidth, CoreConfig.dataWidth));
  interconnect.io.ins <> VecInit(Seq(memMux_0.io.outs(1), memMux_1.io.outs(1)))
  ...
  // AXI Crossbar
  val axiMux = Module(new AXIMux(AXIConfig(dataWidth = 512), 3))
  axiMux.io.ins <> VecInit(icache.io.axi, mem_to_axi_0.io.axi, mem_to_axi_1.io.axi)
