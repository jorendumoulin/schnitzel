...
/** Generic PHS accelerator parameterized by PhsAcceleratorConfig.
  *
  * When the config has a BlackBox (svPath + moduleName set), the datapath
  * is an external PHS-generated SystemVerilog module. Otherwise, falls back
  * to the built-in AluArray for backwards compatibility.
  *
  * CSR base address is 0 — the CsrDemux in Cluster handles address routing
  * and rebasing, so software still writes to the same absolute addresses.
  */
class PhsAccelerator(addrWidth: Int, dataWidth: Int, config: PhsAcceleratorConfig) extends Module {
  ...
  // ---- CSR Interface ----
  // baseAddr = 0 because CsrDemux rebases addresses before they reach here
  val csrItf = Module(new CsrInterface(config.numCsrRegs, 0))
  ...
  def getConfig: PhsWrapper = PhsWrapper("phs", config)
}
